...
function Appointments() {
  ...
  const handleSaveAppointment = (e) => {
    ...
    if (editingApptId) {
      ...
      const newAppt = { id: Date.now(), title: formData.title.trim(), category: formData.category, date: formData.date, time: formData.time, location: formData.location.trim(), notes: formData.notes.trim(), status: formData.status };
  ...
  // --------------------------------------------------------
  const containerStyle = { padding: "24px", maxWidth: "1280px", margin: "0 auto", fontFamily: "'Plus Jakarta Sans', system-ui, -apple-system, sans-serif", color: "#1e232d" };

  const headerRowStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "16px" };

  const pageTitleStyle = { fontSize: "28px", fontWeight: "700", color: "#1e232d", margin: 0 };

  const pageSubtitleStyle = { fontSize: "14px", color: "#747782", marginTop: "4px" };

  // Primary Action Button (#06A9C9)
  const primaryButtonStyle = { backgroundColor: "#06A9C9", color: "#ffffff", border: "none", borderRadius: "10px", padding: "10px 20px", fontSize: "14px", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", boxShadow: "0 2px 8px rgba(6, 169, 201, 0.3)" };

  const statsGridStyle = { display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "28px" };

  const tabBarStyle = { display: "flex", gap: "8px", marginBottom: "20px", borderBottom: "1px solid #e2e4e8", paddingBottom: "8px", overflowX: "auto" };
  ...
  const controlBarStyle = { backgroundColor: "#ffffff", padding: "16px 20px", borderRadius: "12px", border: "1px solid #e2e4e8", boxShadow: "0 1px 4px rgba(47, 129, 150, 0.05)", marginBottom: "20px", display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center", justifyContent: "space-between" };

  const inputGroupStyle = { display: "flex", gap: "12px", flexWrap: "wrap", flex: "1 1 500px" };

  const searchInputStyle = { flex: "1 1 200px", padding: "10px 14px", borderRadius: "8px", border: "1px solid #cbd0d8", fontSize: "14px", outline: "none" };

  const selectStyle = { padding: "10px 14px", borderRadius: "8px", border: "1px solid #cbd0d8", fontSize: "14px", backgroundColor: "#ffffff", outline: "none", cursor: "pointer" };

  const cardsGridStyle = { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "16px" };

  const apptCardStyle = { backgroundColor: "#ffffff", borderRadius: "14px", padding: "20px", border: "1px solid #e2e4e8", boxShadow: "0 2px 6px rgba(47, 129, 150, 0.06)", display: "flex", flexDirection: "column", justifyContent: "space-between" };
  ...
  const modalBackdropStyle = { position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(30, 35, 45, 0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "16px" };

  const modalBoxStyle = { backgroundColor: "#ffffff", borderRadius: "16px", width: "100%", maxWidth: "520px", boxShadow: "0 20px 25px -5px rgba(47, 129, 150, 0.15)", overflow: "hidden" };

  const modalHeaderStyle = { padding: "20px 24px", borderBottom: "1px solid #e2e4e8", display: "flex", justifyContent: "space-between", alignItems: "center" };

  const modalBodyStyle = { padding: "24px" };

  const formGroupStyle = { marginBottom: "18px" };

  const labelStyle = { display: "block", fontSize: "13px", fontWeight: "600", color: "#2F8196", marginBottom: "6px" };

  const inputStyle = { width: "100%", padding: "10px 12px", borderRadius: "8px", border: "1px solid #cbd0d8", fontSize: "14px", boxSizing: "border-box", outline: "none" };
  ...
  return (
    <div style={containerStyle}>
      ...
      {filteredAppointments.length === 0 ? (
        <div style={{ backgroundColor: "#ffffff", borderRadius: "14px", border: "1px solid #e2e4e8", padding: "48px 24px", textAlign: "center", color: "#747782" }}>
        ...
        <div style={cardsGridStyle}>
          {filteredAppointments.map((appt) => {
            ...
            return (
              <div
                key={appt.id}
                style={{ ...apptCardStyle, borderLeft: isToday ? "4px solid #06A9C9" : isCompleted ? "4px solid #2F8196" : "4px solid #6176AC" }}
              >
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                    <span style={{ backgroundColor: "#eaf3f5", color: "#2F8196", padding: "3px 8px", borderRadius: "6px", fontSize: "12px", fontWeight: "600" }}>
                      {appt.category}
                    </span>

                    {isCompleted ? (
                      <span style={{ backgroundColor: "#dcfce7", color: "#16a34a", padding: "3px 10px", borderRadius: "20px", fontSize: "12px", fontWeight: "600" }}>
                        ✓ Completed
                      </span>
                    ) : isToday ? (
                      <span style={{ backgroundColor: "#e6f8fa", color: "#06A9C9", padding: "3px 10px", borderRadius: "20px", fontSize: "12px", fontWeight: "700" }}>
                        ● Today
                      </span>
                    ) : (
                      <span style={{ backgroundColor: "#edf0f8", color: "#6176AC", padding: "3px 10px", borderRadius: "20px", fontSize: "12px", fontWeight: "600" }}>
                        ⏳ Upcoming
                      </span>
                    )}
                  </div>

                  <h3 style={{ fontSize: "16px", fontWeight: "700", color: "#1e232d", margin: "0 0 10px 0", textDecoration: isCompleted ? "line-through" : "none", opacity: isCompleted ? 0.75 : 1 }}>
                    {appt.title}
                  </h3>

                  <div style={{ display: "flex", alignItems: "center", gap: "12px", fontSize: "13px", color: "#2F8196", marginBottom: "8px", fontWeight: "500" }}>
                    <div>📅 {formatDisplayDate(appt.date)}</div>
                    <div>⏰ {formatDisplayTime(appt.time)}</div>
                  </div>

                  {appt.location && (
                    <div style={{ fontSize: "13px", color: "#747782", marginBottom: "8px", display: "flex", alignItems: "center", gap: "6px" }}>
                      <span>📍</span>
                  ...
                  {appt.notes && (
                    <p style={{ fontSize: "12px", color: "#747782", backgroundColor: "#f4f5f7", padding: "8px 10px", borderRadius: "6px", margin: "10px 0 0 0", borderLeft: "2px solid #cbd0d8" }}>
                      {appt.notes}
                    </p>
                  )}
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "18px", paddingTop: "12px", borderTop: "1px solid #f1f3f5" }}>
                  <button
            ...
            <form onSubmit={handleSaveAppointment} style={modalBodyStyle}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "24px" }}>
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  style={{ backgroundColor: "#f4f5f7", color: "#747782", border: "none", padding: "10px 18px", borderRadius: "8px", fontWeight: "600", cursor: "pointer" }}
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  style={{ backgroundColor: "#06A9C9", color: "#ffffff", border: "none", padding: "10px 20px", borderRadius: "8px", fontWeight: "600", cursor: "pointer", boxShadow: "0 2px 6px rgba(6, 169, 201, 0.25)" }}
                >
          ...
          <div style={{ ...modalBoxStyle, maxWidth: "400px", padding: "24px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
              ...
              <button
                ...
                style={{ backgroundColor: "#dc2626", color: "#ffffff", border: "none", padding: "10px 18px", borderRadius: "8px", fontWeight: "600", cursor: "pointer" }}
              >
                Yes, Delete
              </button>
            </div>
